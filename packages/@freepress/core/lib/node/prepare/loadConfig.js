...
const { deepMerge } = require('@freepress/util');
...
module.exports = function loadConfig(freepressDir, bustCache = true) {
  const configPath = path.resolve(freepressDir, 'config.js');
  const configYmlPath = path.resolve(freepressDir, 'config.yml');
  const configTomlPath = path.resolve(freepressDir, 'config.toml');
