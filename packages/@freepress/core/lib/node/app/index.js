...
} = require('@freepress/webpack');
const { applyUserWebpackConfig, logger, resolveAddress, getCurrentTime } = require('@freepress/util');
...
class App {
  ...
  async prepare() {
    ...
    const stylePath = path.resolve(sourceDir, '.freepress/style.less').replace(/[\\]+/g, '/');
    ...
    // const enhanceAppPath = path.resolve(sourceDir, '.freepress/enhanceApp.js');
  ...
  async createWebpackConfig(isServer, isProd) {
    ...
    // registry log plugin
    clientConfig.plugin('freepress-log').use(WebpackLogPlugin);
  ...
  async createDeveServer() {
    ...
    const publicPath = path.resolve(sourceDir, '.freepress/public');
    ...
    const serverConfig = Object.assign(
      {
        ...
        watchOptions: {
          ignored: [/node_modules(\\|\/)(?!@freepress(\\|\/)core(\\|\/).temp)/]
        },
        historyApiFallback: {
          disableDotRule: true,
          rewrites: [
  ...
  async createSSRServer() {
    ...
    const contentBase = path.resolve(options.sourceDir, '.freepress/public');
  ...
  serverReady() {
    ...
    logger.success(
      `freepress server listening at ${chalk.cyan(`http://${displayHost}:${port}${base}\n`)}`
    );
  }
  ...
  async eject() {
    ...
    const target = path.resolve(dir, '.freepress/theme');
