...
const App = require('@freepress/core/lib/node/app');
const { logger } = require('@freepress/util');

if (!semver.satisfies(process.version, requiredVersion)) {
  logger.error(
    `\n[freepress] minimum Node version not met:` +
...
program
  ...
  .option('-d, --dest <outDir>', 'specify build output dir (default: .freepress/dist)')
...
program
  ...
  .description('copy the default theme into .freepress/theme for customization.')
  .action((dir = 'docs') => {
...
program.on('--help', () => {
  console.log();
  console.log(
    `  Run ${chalk.cyan(`freepress <command> --help`)} for detailed usage of given command.`
  );
  console.log();
});
