...
const HIGHLIGHT_DIRECTIVE = createDirectiveRegExp(
  `highlight`
);
const END_DIRECTIVE = {
...
const highlightWrap = line =>
  [
    `<span class="freepress-highlight-code-line">`,
    line,
    `</span>`
  ].join(``); // const wrapAndStripComment = line => wrap(stripComment(line))

const parseLine = (line, code, index, actions) => {
  const [, feature, directive, directiveRange] = line.match(
    DIRECTIVE
  );
  const flagSource = {
    feature,
  ...
  switch (directive) {
    ...
    case `start`: {
      ...
      const endIndex = code.findIndex(
        (line, idx) =>
          idx > index && END_DIRECTIVE[feature].test(line)
      );
    ...
    case `range`:
      ...
      if (directiveRange) {
        const strippedDirectiveRange = directiveRange.slice(
          1,
          -1
        );
        const range = rangeParser.parse(
          strippedDirectiveRange
        );
...
module.exports = function highlightLineRange(
  code,
  highlights = []
) {
  if (
    highlights.length > 0 ||
    HIGHLIGHT_DIRECTIVE.test(code)
  ) {
